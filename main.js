...
var mainState = {
    ...
    jump: function() {
        if(this.gameStarted) {
            ...
            var titleFade = game.add.tween(this.gameTitle1).to({alpha:0}, 1000).start();
            var titleFade2 = game.add.tween(this.gameTitle2).to({alpha:0}, 1000).start();
            this.startGame();
        }
    },
    hitPipe: function() {
    ...
    endGame: function() {
        ...
        labelScore = game.add.text(200, 290, "High Score: " + localStorage.getItem('highScore'), 
            { font: "25px Helvetica", fill: "#ffffff" }); 
